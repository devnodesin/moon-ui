...
- For external Moon backends, see <https://moon.devnodes.in/doc/llms.md>
- Moon github repo: <https://github.com/devnodesin/moon>

## BACKEND (MOON SERVER)

- Testing server: `https://moon.devnodes.in/`
...
- All `curl` examples in `https://moon.devnodes.in/doc/llms.md` and `https://moon.devnodes.in/doc/llms.json` are reference snippets for humans and agents only — the application must perform equivalent REST calls from the browser, not by invoking `curl`.
...
- Follow session timeout and forced-logout rules from  and `https://moon.devnodes.in/doc/llms.json` when validating/refreshing tokens.
...
- Follow session timeout and auto-logout rules as specified in  and `https://moon.devnodes.in/doc/llms.json` (enforce via the app HTTP client).
