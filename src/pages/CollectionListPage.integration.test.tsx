...
describe('Collections Page - API Integration Test', () => {
  ...
  it('should work with OLD API format for backward compatibility', async () => {
    // Mock collections list with OLD API format (object array with columns)
    mock.onGet(`${BASE_URL}/collections:list`).reply(200, {
      collections: [
        {
          name: 'products',
    ...
    // Mock schema endpoint with OLD API format (unwrapped array)
    mock.onGet(`${BASE_URL}/products:schema`).reply(200, [
      { name: 'id', type: 'string', nullable: false },
      { name: 'name', type: 'string', nullable: false },
    ]);
